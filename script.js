...
UI.input[0].addEventListener('focus', () => {
	UI.input[0].addEventListener('keyup', () => {
		convertFromEuro();
	});
});
UI.input[1].addEventListener('focus', () => {
	UI.input[1].addEventListener('keyup', () => {
		convertFromDollar();
	});
});
UI.input[2].addEventListener('focus', () => {
	UI.input[2].addEventListener('keyup', () => {
		convertFromPound();
	});
});
...
function convertFromEuro() {
	if (!UI.input[0].value) {
		...
		UI.input[1].value = (UI.input[0].value * 1.2).toFixed(2);
		UI.input[2].value = (UI.input[0].value * 0.9).toFixed(2);
	}
}
